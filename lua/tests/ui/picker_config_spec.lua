...
-- -- File picker (rg --hidden) -- --

T["file picker rg"] = MiniTest.new_set()

T["file picker rg"]["rg --hidden finds dotfiles"] = function()
    local tmpdir = vim.fn.tempname() .. "_rg_hidden"
    vim.fn.mkdir(tmpdir .. "/.github/workflows", "p")
    ...
    local has_visible = stdout:find("visible%.txt") ~= nil

    vim.fn.delete(tmpdir, "rf")

    MiniTest.expect.equality(has_hidden, true, "rg --hidden should find .github/workflows/ci.yml")
    MiniTest.expect.equality(has_visible, true, "rg --hidden should find visible.txt")
end

T["file picker rg"]["rg --hidden respects gitignore"] = function()
    local tmpdir = vim.fn.tempname() .. "_rg_gitignore"
    vim.fn.mkdir(tmpdir .. "/ignored_dir", "p")
    vim.fn.writefile({ "ignored_dir/" }, tmpdir .. "/.gitignore")
    vim.fn.writefile({ "should not appear" }, tmpdir .. "/ignored_dir/secret.txt")
    vim.fn.writefile({ "should appear" }, tmpdir .. "/kept.txt")
    ...
    local has_kept = stdout:find("kept%.txt") ~= nil
    local has_gitignore = stdout:find("%.gitignore") ~= nil

    vim.fn.delete(tmpdir, "rf")

    MiniTest.expect.equality(has_ignored, false, "rg should exclude gitignored files")
    MiniTest.expect.equality(has_kept, true, "rg should include non-ignored files")
    MiniTest.expect.equality(has_gitignore, true, "rg --hidden should include .gitignore itself")
end

T["file picker rg"]["rg excludes .git directory"] = function()
    local tmpdir = vim.fn.tempname() .. "_rg_nogit"
    vim.fn.mkdir(tmpdir, "p")
    vim.fn.writefile({ "content" }, tmpdir .. "/file.txt")
    ...
    MiniTest.expect.equality(has_git_internal, false, "rg --hidden should still exclude .git/ directory contents")
end
...
T["grep globs"] = MiniTest.new_set()

T["grep globs"]["rg glob restricts to matching files"] = function()
    local tmpdir = vim.fn.tempname() .. "_rg_glob"
    vim.fn.mkdir(tmpdir, "p")
    vim.fn.writefile({ "hello world" }, tmpdir .. "/file.lua")
    ...
    local stdout = result.stdout or ""
    local has_lua = stdout:find("file%.lua") ~= nil
    local has_txt = stdout:find("file%.txt") ~= nil

    vim.fn.delete(tmpdir, "rf")

    MiniTest.expect.equality(has_lua, true, "Glob *.lua should include .lua files")
    MiniTest.expect.equality(has_txt, false, "Glob *.lua should exclude .txt files")
end

T["grep globs"]["multiple globs combine correctly"] = function()
    local tmpdir = vim.fn.tempname() .. "_rg_multiglob"
    vim.fn.mkdir(tmpdir, "p")
    vim.fn.writefile({ "match" }, tmpdir .. "/a.lua")
    vim.fn.writefile({ "match" }, tmpdir .. "/b.py")
    ...
    MiniTest.expect.equality(stdout:find("a%.lua") ~= nil, true)
    MiniTest.expect.equality(stdout:find("b%.py") ~= nil, true)
    MiniTest.expect.equality(stdout:find("c%.txt") ~= nil, false, "txt should be excluded by globs")
end
