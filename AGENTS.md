...
This file explains conventions and the reasoning behind them. Keep it evergreen: point at the source of truth (`mise tasks`, `hk.pkl`, a directory, a module) instead of copying lists of tasks, files, commands, or keybindings that drift the moment the code changes. Record a fact here only when it is non-obvious from reading the code.
...
Git hooks run through [hk](https://hk.jdx.dev/), configured in `hk.pkl`, so `mise run fmt` shells out to `hk fix --all`. Install them once with `hk install --mise`, which is also what makes hook steps resolve the mise-managed tools in `[tools]`. Escape hatches: `HK=0` for one command, or `git ... --no-verify`.
...
mise run doc:vimdoc
# Or directly: ./scripts/panvimdoc.sh

# The `panvimdoc` hk step regenerates it on commit whenever doc/src/ changes
```
...
Output is `doc/kyleking-neovim.txt` (`:h kyleking-neovim`), with `doc/tags` regenerated by the `helptags` step. panvimdoc ships only as a git repo of shell and pandoc-lua scripts, so `scripts/panvimdoc.sh` clones it at a pinned tag into the gitignored `deps/` directory and needs `pandoc` on `PATH`. That is also why the doc-generating steps sit in CI's `HK_SKIP_STEPS` list.

Markdown outside `doc/src/` and `doc/generated/` is standalone prose that never reaches the vimdoc build, and mdformat treats the two sets differently (see the `mdformat` step's `exclude` in `hk.pkl`).

### Linting and formatting

`mise run fmt` runs every fix step; `mise run fmt:<name>` runs one of them when you want a fast loop on a single tool, and `hk fix --all --step <name>` covers any step without a task. `hk.pkl` is the source of truth for which steps exist and what each one covers, and `hk check --plan` or `hk check --why <step>` explains why a step did or did not run.

Steps land in a hook by which local mapping they are spread into: `linters` runs everywhere, `codegen` only on commit, and `pre_push_checks` (the whole test suite, workspace Lua diagnostics, startup measurement) only on push. Run `mise run check` before pushing, since committing never exercises that last group.

Several hand-rolled steps exist because hk ships no builtin for them, and each carries a comment saying why: `check-vcs-permalinks` (a grep stand-in), `jq` (amended for 4-space indent, matching the old `pretty-format-json` hook), and `mdformat` (run through `uvx` with its plugins). `tombi-format` replaced both `check-toml` and `toml-sort`, and `shfmt` replaced `prettier-plugin-sh`. pre-commit's `destroyed-symlinks` and `forbid-new-submodules` were dropped as noise for a personal config.
...
The `fixture:*` tasks run the fixtures, refresh snapshots, and profile per-fixture timing; `mise run doc:plugin` regenerates the markdown and also runs from the `generate-plugin-docs` hk step whenever a file under `lua/tests/docs/` changes.
...
`.github/workflows/test.yml` runs the CI-safe suite against both stable and nightly Neovim, then runs `hk check --all` with an `HK_SKIP_STEPS` list for steps that need nvim, pandoc, or lua-language-server. Adding a step that depends on a tool the runner cannot install means adding it to that list, or CI fails. Tools the lint job does need belong in `[tools]` in `mise.toml`, which `jdx/mise-action` installs.

### Lua diagnostics (lua-language-server)

The `lua-diagnostics` step (`scripts/check-lua-lsp.sh`) runs only on `pre-push` and is in CI's `HK_SKIP_STEPS` list, so it never runs automatically until push time -- `mise run check` before pushing is the only routine check. Run `lua-language-server --check=$(pwd) --check_format=pretty --checklevel=Warning` directly for a faster loop while fixing a batch of warnings.
