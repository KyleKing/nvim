...
T["mini.ai"]["n_lines config is respected"] = function()
    ...
    MiniTest.expect.equality(MiniAi.config.n_lines, 500, { fail_reason = "n_lines should be set to 500" })
end

T["mini.ai"]["search_method config is set"] = function()
    local MiniAi = require("mini.ai")
    MiniTest.expect.equality(
        MiniAi.config.search_method,
        "cover_or_next",
        { fail_reason = "search_method should be 'cover_or_next'" }
    )
end
