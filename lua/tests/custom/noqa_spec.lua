...
T["tool configs"]["all expected tools configured"] = function()
    ...
    for _, tool in ipairs(expected) do
        MiniTest.expect.equality(noqa._tool_configs[tool] ~= nil, true, { fail_reason = tool })
    end
end
...
T["tool configs"]["templates generate valid output"] = function()
    for name, config in pairs(noqa._tool_configs) do
        ...
        MiniTest.expect.equality(type(result), "string", { fail_reason = name })
        MiniTest.expect.equality(#result > 0, true, { fail_reason = name .. " should generate non-empty string" })
    end
end
