...
local function parse_list_line(line)
    ...
    local number, punct
    indent, number, punct, content = line:match("^(%s*)(%d+)([%.%)])%s+(.*)$")
    if number then return number .. punct, #indent, content, "ordered" end
...
function M.handle_return()
    ...
    if content == "" or is_blank(content) then
        -- Buffer changes are disallowed while an expr-mapping is being evaluated (E565),
        -- so defer the marker deletion until after this function returns.
        vim.schedule(function()
            vim.api.nvim_set_current_line(string.rep(" ", indent_len))
            vim.api.nvim_win_set_cursor(0, { vim.api.nvim_win_get_cursor(0)[1], indent_len })
        end)
        return ""
    end
